...
//    }
    ...
    NSString *plistPath = [documentsPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.plist", self.currentThemeName]];

    if (plistPath != nil) {
        styles = [NSDictionary dictionaryWithContentsOfFile:plistPath];
        
        if (styles == nil) {
            NSString *path = [[NSBundle mainBundle] pathForResource:self.currentThemeName ofType:@"plist"];
            styles = [NSDictionary dictionaryWithContentsOfFile:path];
        }
    }
    else {
        NSString *path = [[NSBundle mainBundle] pathForResource:self.currentThemeName ofType:@"plist"];
